...
<body>
        ...
        <form style="margin-top: 50px;" class="form-inline" action="{{ route('admin.prof.index') }}" method="GET">
...
<Div class="container mt-5">
        <div class="row">
            <div class="col-md-12">
                ...
                <a href="{{ route('admin.prof.create') }}" class="btn btn-success" style="float: right;">Add tuteur</a>
                <div class="card-header">Professeur List</div>
                        ...
                        <table class="table">
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <th scope="col">Action<th scope="col"></th></th>
                                </tr>
                            </thead>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td><img class="border rounded-circle p-2 mx-auto mb-3" style="width: 70px; height:70px;" src="{{ asset('uploads/prof/'.$ap->img)}}" alt="Avatar" heigth="50"></td>
                                    <td>{{$ap->name }}</td>
                                    <td>{{$ap->phone}}</td>
                                    <td>{{$ap->spec}} </td>
                                             <td>
                                        <td> <a href="{{route('admin.prof.edit',$ap->id)}}" class="btn btn-warning"><i class="fas fa-edit"></i></a></td>
                                        <td>
                                            <form action="{{ route('admin.prof.delete', $ap->id) }}" method="POST">
                                                @csrf
                                                @method('POST')
                                            ...
                                            <script>
                                                window.deleteConfirm = function (e) {
                                                    ...
                                                    Swal.fire({
                                                        title: "Are you sure?",
                                                        text: "Once deleted, you will not be able to recover the profeseur" + "{{ $ap->name }}" + "!",
                                                        icon: "warning",
  ...
  <div>
    @else
    <tr> <STrong> <p>Tuteur not found</p></tr></STrong> </TR>
      @endif
   <script>
